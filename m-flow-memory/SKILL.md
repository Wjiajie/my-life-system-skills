...
name: m-flow-memory
description: >
  跨会话持久化记忆系统，基于 M-flow 认知记忆引擎（图路由检索，远优于 RAG）。
  通过 MCP 工具 memorize/search/query 实现结构化知识的写入和召回。

  使用此 Skill 的场景：
  1. 用户说"记住..."、"帮我记一下..."、"下次别忘了..."
  2. 用户问"我之前/上次/以前..."（跨会话记忆召回）
  3. 重要决策、项目约定、架构设计需要持久化时
  4. 对话过程中发现重要信息，主动判断并征询用户确认（见主动记忆判断协议）
  5. 用户问"你还记得..."（验证记忆是否存在）
  6. 用户要求管理/查看/删除已有记忆
