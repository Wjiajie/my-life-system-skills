...
3. **Git-Based Engineering Metrics** *(Inspired by GStack /retro)*

   In addition to chat history analysis, gather quantitative engineering metrics from Git to provide a data-driven view of the user's output:

   ```powershell
   # Commits in the past 7 days
   git log --since="7 days ago" --format="%H|%aN|%ai|%s" --shortstat

   # Per-commit LOC breakdown (test vs production)
   git log --since="7 days ago" --format="COMMIT:%H|%aN" --numstat

   # Commit time distribution (for session detection)
   git log --since="7 days ago" --format="%at|%aN|%ai|%s"

   # Hotspot files (most frequently changed)
   git log --since="7 days ago" --format="" --name-only | sort | group -NoElement | sort Count -Descending | select -First 10
   ```
   ...
   **Compute and report:**
   ...
   | Test LOC Ratio | Test file lines / Total lines |
   | Active Days | Distinct dates with commits |
   | Work Sessions | 45-min gap threshold between commits |
   | Avg LOC/Session-Hour | Net LOC / Total session hours |

   **Session Detection**: Use 45-minute gap threshold between consecutive commits:
   - **Deep sessions** (50+ min): Focused deep work
   - **Medium sessions** (20-50 min): Standard work blocks
   - **Micro sessions** (<20 min): Quick fixes, fire-and-forget

   **Commit Type Breakdown**: Categorize by conventional commit prefix:
   ```
   feat:     12  (40%)  ████████████████████
   fix:       9  (30%)  ███████████████
   refactor:  5  (17%)  ████████
   ...
   **Commit Time Distribution**: Show hourly histogram to identify:
   - Peak productivity hours
   - Dead zones
   - Late-night coding clusters (after 10pm — flag as sustainability concern)

   **Hotspot Analysis**: Files changed most frequently may indicate:
   - Unstable interfaces needing refactoring
   - Core modules that need more test coverage
   - Areas where the architecture may need improvement
...
6. **Search for Learning Resources**
