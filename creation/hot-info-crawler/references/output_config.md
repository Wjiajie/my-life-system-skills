...
本技能支持将检索结果**增量写入**本地 Markdown 文件，每完成一个板块立即保存，防止因网络中断或上下文溢出导致信息丢失。完成后可按当前用户请求启动 Obsidian，并在明确授权时发布到博客仓库。
...
| `blogPublish.autoPush` | ❌ | 当前请求明确授权发布时是否一并 push，默认 `false`；配置本身不构成 push 授权 |
...
在创建配置前，询问用户是否保存博客发布目标。保存配置不会自动授权未来的 commit 或 push：
...
抓取完成后（写入 `<!-- section:all_done -->` 后），仅在当前请求包含打开或同步 Obsidian 的意图时唤起应用：
...
如果 `config.json` 配置了 `blogPublish.enabled = true`，并且当前用户请求明确要求发布，才执行博客发布流程。开始前检查目标仓库、当前分支和 dirty scope；不得把无关修改一起提交。
...
`src/content/feeds` 目录的 Astro collection 需要 frontmatter。使用当前宿主的文件编辑能力在文件最顶部插入：
...
### 步骤 4：git push（仅当当前请求已明确授权且 autoPush = true）
