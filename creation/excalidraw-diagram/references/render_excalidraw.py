...
def command_hint(command: str) -> str:
    return f'Run: cd "{SCRIPT_DIR}" && {command}'
...
def render(
    ...
    try:
        ...
        print(command_hint("uv sync && uv run playwright install chromium"), file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(command_hint("uv run playwright install chromium"), file=sys.stderr)
                sys.exit(1)
            raise
