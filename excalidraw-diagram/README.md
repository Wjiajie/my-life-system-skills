...
For hand-drawn whiteboards with editable concept icons, the bundled offline catalog can be searched and inserted directly:

```bash
python scripts/search_icons.py "workflow 流程"
python scripts/add_icon_to_diagram.py diagram.excalidraw workflow 120 320 --width 180
...
After a diagram has been rendered and validated, upload it for a shareable Excalidraw link:

```bash
uv run --project references python scripts/upload.py <path-to-file.excalidraw>
```

The upload is encrypted client-side before it is sent to Excalidraw's public JSON endpoint.
...
excalidraw-diagram/
  ...
  scripts/
    split_excalidraw_library.py    # Index any .excalidrawlib
    search_icons.py                # Semantic keyword search across catalogs
    add_icon_to_diagram.py         # Insert editable native icon elements
    apply_style_preset.py          # Normalize hand-drawn whiteboard style
    validate_scene.py              # Validate IDs, bindings, and aspect ratio
    upload.py                       # Encrypted upload to excalidraw.com
  assets/style-presets/             # Reusable visual contracts
  libraries/core-handdrawn/         # Bundled offline native icon starter set
  references/
