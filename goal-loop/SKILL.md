...
2. Use `$plan-eng-review` to produce an executable plan and TODO list, including the Subagent Fit Check when the task can be decomposed.
...
Native Goal mode describes the final condition for the session. It does not replace `$plan-eng-review`, `$qa`, handoff rules, or the final `GOAL_LOOP_STATUS` contract.
