...
不要继续实现代码。请按紧凑、引用优先的结构保存上下文。此处是 goal-loop handoff 场景，必须写入 goal-loop 专用位置，不要写入其他 skill 的默认目录。

请执行：
1. 整理 goal-loop 交接上下文，包含：
   - 当前目标
   - repo / branch / 当前工作树状态
   - 已完成事项
   - 关键决策
...
2. 检查当前项目并更新执行方案和 TODO；模块边界不清时使用 $codebase-design，并仅在 delegation 可用且已获授权时判断是否启动 subagent。
3. 执行剩余 TODO；代码变更存在合适行为 seam 时使用 $tdd。
