...
如果这是新窗口恢复，请先读取该交接文件；不要继承文件之外的旧目标、旧 TODO 或旧验证结论，再继续执行。
"""
...
2. 检查当前项目并形成可拆解执行方案和 TODO 清单；模块边界不清时使用 $codebase-design，并仅在 delegation 可用且已获授权时判断是否启动 subagent。
3. 执行 TODO；代码变更存在合适行为 seam 时使用 $tdd。
4. 运行相关测试、静态检查和运行时证据；非显然失败使用 $diagnosing-bugs，收口使用 $code-review。
...
当 `handoff_status: unstable` 时，hook 会要求按紧凑、引用优先的交接结构将上下文保存到 `.goal-loop/handoffs/`，然后提示用户新开窗口继续。
{handoff_context}
"""
